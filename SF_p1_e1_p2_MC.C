...
double    P_max = 1;            //Upper bound for P1 in GeV
double    P2_min = 0;           //Lower bound for P2
int       nbins = 100;          //Number of bins  of the PR Wavefunction file
int       nbinsp1 = 1000;         //Number of bins for the P1 Spectrum
int       nbinsp2 = 1000;         //Number of bins for the P2 Spectrum
int       nbinse = 1000;         //Number of bins for the Energy Spectrum
...
TFile f1("Contact_formalism.root", "RECREATE");
    ...
    */
    ...
    TH3D*   Histo_SF_nn = new TH3D("Spectral Function nn","Spectral Function nn",nbinsp1,0,P_max,nbinsp2,P2_min,P_max,nbinse,0,m);
   TH3D*   Histo_SF_pn0 = new TH3D("Spectral Function pn0","Spectral Function pn0",nbinsp1,kf,P_max,nbinsp2,P2_min,P_max,nbinse,0,m);
    //TH3D*   Histo_SF_pn1 = new TH3D("Spectral Function pn1","Spectral Function pn1",nbinsp1,kf,P_max,nbinsp2,P2_min,P_max,nbinse,0,m);
    //TH1D*   Histo_Missed = new TH1D("Missed","Missed",200,-10,10);
    ...
    */
    ...
    double dp2 = (P_max-P2_min)/(double)nbinsp2;
       ...
       // p2_stop = ((p1-P2_min)/dp2)+1;                               //I require p2<p1
        ...
        p2 = dp2 * ((double)(p2_i + 0.5)) + P2_min;
               ...
               if(E1_stop>nbinse){E1_stop=nbinse;}
                   ...
                   for(E1_i=E1_start; E1_i<E1_stop; E1_i++){
               ...
               E1 = dE1 * ((double)(E1_i + 0.5));
                       ...
                       if (P_CM2 <=0 || Cos_Theta_num < -(2*p1*p2) || Cos_Theta_num  > 2*p1*p2 || P_R2<=(kf*kf) ||P_R2>100){
                            ...
                            Weight_nn = p2/p1 * m  / (P_R2*P_R2) / (  sigma_P_CM* sigma_P_CM* sigma_P_CM) * TMath::Exp((-1)*P_CM2/(2* sigma_P_CM*sigma_P_CM) );
                            
                            
                            /*
                            
                            Weight_nn = p2/p1 * m  * (histo_p_nn -> GetBinContent((int)((sqrt(P_R2)/k[0]+0.5)))) / (  sigma_P_CM* sigma_P_CM* sigma_P_CM) * TMath::Exp((-1)*P_CM2/(2* sigma_P_CM*sigma_P_CM) );
                        ...
                        Histo_SF_nn ->Fill (p1, p2, E1, Weight_nn);
                        //Histo_SF_pn0 ->Fill (p1, p2, E1, Weight_pn0);
                        //Histo_SF_pn1 ->Fill (p1, p2, E1, Weight_pn1);
   ...
   // Histo_PCM_pn0->Scale(1/Histo_PCM_pn0->Integral(),"width");
    ...
    TH1D* Histo_E1_nn = Histo_SF_nn->ProjectionZ("#epsilon1",0,-1,0,-1);
    Histo_E1_nn->SetTitle("#epsilon1");
    Histo_E1_nn->GetXaxis()->SetTitle("#epsilon1 (GeV)");
    Histo_E1_nn->GetYaxis()->SetTitle("S_{#epsilon1} (a.u)");

    
   // TH1D* Histo_E1_pn0 = Histo_SF_pn0->ProjectionZ("Histo_E1_pn0",0,-1,0,-1);
    //TH1D* Histo_E1_pn1 = Histo_SF_pn1->ProjectionZ("Histo_E1_pn1",0,-1,0,-1);
  ...
  //  legendE->Draw();
   ...
   // legendtheta->Draw();
    ...
    //For the momentum
    TH1D* Histo_p1_nn = Histo_SF_nn->ProjectionX("P1",0,-1,0,-1);
    Histo_p1_nn->SetTitle("P_{1}");
    Histo_p1_nn->GetXaxis()->SetTitle("P_{1} (GeV/c)");
    Histo_p1_nn->GetYaxis()->SetTitle("S_{P_{1}} (a.u)");
    
    //TH1D* Histo_p1_pn0 = Histo_SF_pn0->ProjectionX("Histo_p1_pn0",0,-1,0,-1);
    //TH1D* Histo_p1_pn1 = Histo_SF_pn1->ProjectionX("Histo_p1_pn1",0,-1,0,-1);
    ...
    TH1D* Histo_p2_nn = Histo_SF_nn->ProjectionY("P2",0,-1,0,-1);
   ...
   // Histo_SF_nn -> Write("3D_Spectral_Function_nn");
    Histo_p1_nn -> Write("P1_nn_3");
    Histo_p2_nn -> Write("P2_nn_3");
    Histo_E1_nn -> Write("E1_nn_3");
    Histo_PR_nn -> Write("PR_nn_3");
    Histo_PCM_nn-> Write("PCM_nn_3");
    Histo_Cos_theta_nn -> Write("Cos_theta_nn_3");
